 ...
 *
 * A Reading Position belongs to one Book, so `/recent` is registered before
 * `/:bookId` and the upsert below is keyed on `book_id` alone. `PUT` writes the
 * position the reader is at, which may move backwards — it is not a
 * furthest-read marker.
 */
import type { Response } from 'express';
import { Router } from 'express';
import type { ReadingPositionResponse, RecentReadingResponse } from '@lan-reader/shared';
