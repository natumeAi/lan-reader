...
export function useFoliateReader(options: Options) {
  ...
  useEffect(() => {
    ...
    const displayPosition = (position: StablePosition, persist: boolean) => {
      if (disposed || !owned || owned.state !== 'ready') return;
    ...
    void (async () => {
      try {
        ...
        owned.onPosition = position => displayPosition(position, true);
        ...
        owned.onState = (state, error) => {
          ...
          if (state === 'ready') { setError(''); setIsLoading(false); if (owned?.stable) displayPosition(owned.stable, false); }
        };
        await owned.open(data, target);
        if (disposed) return;
        setToc(owned.toc); pageProgressController.setReadingSections(owned.readingSections);
        markReaderSettingsLoaded();
        if (document.visibilityState === 'hidden') owned.suspend();
        capture.current = () => { if (!owned || disposed || owned.state !== 'ready') return false; owned.publish(); return true; };
        if (owned.state === 'ready') { owned.publish(); owned.schedulePages(); }
      } catch (error) {
        if (disposed) return;
  ...
  const requestBookPagination = useCallback(() => engine?.schedulePages(), [engine]);
  const retry = useCallback(() => { setReload(value => value + 1); }, []);
  const startChapter = useCallback(() => { if (fallback.current) { restore.current = fallback.current; setReload(value => value + 1); } }, []);
  return { engine, toc, currentChapter, currentHref, progress, captureCurrentProgress, requestBookPagination, retry, startChapter, canFallback };
}
