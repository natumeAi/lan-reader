...
import type { LibraryView, LibrarySort } from '../utils/libraryView.js';
...
export function useLibraryView({ shelfItems, catalogBooks }: { shelfItems: ShelfItem[]; catalogBooks: CatalogBook[] }) {
  ...
  const [view, setView] = useState<LibraryView>(LIBRARY_VIEW.ALL);
  const [sort, setSort] = useState<LibrarySort>(() => getLibrarySortPreference(
    initialSortPreferences,
    LIBRARY_VIEW.ALL,
  ));
  const [searchFocused, setSearchFocused] = useState(false);
  const searchSnapshotRef = useRef<{ view: LibraryView; sort: LibrarySort } | null>(null);
  ...
  const selectView = useCallback((nextView: LibraryView) => {
  ...
  const selectSort = useCallback((nextSort: LibrarySort) => {
