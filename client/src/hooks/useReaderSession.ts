import type { HydratedLibrarySnapshot, ReaderBook } from '../types/library.js';
...
function historyStateWithReader(bookId: number) {
  return withReaderHistoryState(window.history.state, bookId);
}

function ensureReaderHistoryEntry(bookId: number, { fresh = false } = {}) {
  try {
...
export function useReaderSession() {
  const [readingBook, setReadingBook] = useState<ReaderBook | null>(readActiveReaderBookSnapshot);
  const [readingBookOrigin, setReadingBookOrigin] = useState<MotionRect | null>(null);
  ...
  const openBook = useCallback((book: ReaderBook, originRect?: MotionRect | null, options: { disabled?: boolean } = {}) => {
  ...
  const clearReaderBookIfDeleted = useCallback((bookId: number) => {
  ...
  const restoreReaderBook = useCallback(async (shelfData: HydratedLibrarySnapshot['shelfData'], recentData: HydratedLibrarySnapshot['recentData']) => {
    const activeBookId = readActiveReaderBookId();
