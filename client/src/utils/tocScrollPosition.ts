interface TocEntryGeometry { getBoundingClientRect: () => { top: number; height: number } }
interface TocScrollContainer { getBoundingClientRect: () => { top: number }; clientHeight: number; scrollHeight: number; scrollTop: number }
export function centerTocEntry(container: TocScrollContainer | null | undefined, entry: TocEntryGeometry | null | undefined) {
  if (
    !container ||
    !entry ||
