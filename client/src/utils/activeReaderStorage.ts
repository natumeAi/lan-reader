...
function sanitizeBookSnapshot(value: unknown) {
...
export function writeActiveReaderBookId(bookId: number) {
  try {
...
export function readActiveReaderBookSnapshot() {
  try {
    ...
    const snapshot = sanitizeBookSnapshot(
      JSON.parse(localStorage.getItem(activeReaderBookSnapshotStorageKey) ?? 'null'),
    );
...
export function writeActiveReaderBookSnapshot(book: unknown) {
...
export type ActiveReaderBookSnapshot = NonNullable<ReturnType<typeof sanitizeBookSnapshot>>;
