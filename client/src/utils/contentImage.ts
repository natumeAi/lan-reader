...
// Tag/namespace checks also work for elements owned by an EPUB iframe.
function isHtmlImage(element: Element): element is HTMLImageElement {
  return element.localName === 'img' && element.namespaceURI === 'http://www.w3.org/1999/xhtml';
}
...
function containsPoint(rect: DOMRect | undefined, x: number, y: number) {
  return Boolean(
    rect &&
    rect.width > 0 &&
    rect.height > 0 &&
...
function computedStyleFor(element: Element | null) {
  const view = element?.ownerDocument?.defaultView;
  if (!element || !view?.getComputedStyle) return null;
  try {
...
function isVisibleContentImage(element: Element | null) {
...
function hitsContentImageWhenEnabled(document: Document, element: Element, x: number, y: number) {
  const style = styledElement(element) ? element.style : undefined;
  if (
...
function imageAtPoint(document: Document, x: number, y: number) {
...
function svgDimensions(svg: Element) {
...
function replaceCssUrls(value: string, replaceUrl: (url: string) => string) {
  return String(value || '').replace(
    /url\(\s*(?:"([^"]*)"|'([^']*)'|([^)]*?))\s*\)/gi,
    (match: string, doubleQuoted: string | undefined, singleQuoted: string | undefined, unquoted: string | undefined) => {
...
function createSvgResourceCollector() {
  const resources: SvgImageResource[] = [];
  const placeholdersBySource = new Map<string, string>();

  const register = (value: string, baseUri: string) => {
...
function fontFaceRules(document: Document, register: (url: string, baseUri: string) => string) {
  const rules: string[] = [];
  const visited = new Set<CSSStyleSheet>();

  const visit = (sheet: CSSStyleSheet | null) => {
    ...
    for (const rule of cssRules) {
      ...
      } else if ('styleSheet' in rule && rule.type === 3) {
        visit((rule as CSSImportRule).styleSheet);
      }
    }
  };
...
function serializeSvg(svg: Element) {
  // cloneNode preserves the source Element interface.
...
export async function prepareSvgContentImageSource(contentImage: ContentImage | null | undefined, {
  fetchImpl = globalThis.fetch,
  signal,
}: { fetchImpl?: (url: string, options?: { signal: AbortSignal }) => Promise<Pick<Response, 'ok' | 'blob'>>; signal?: AbortSignal } = {}) {
  ...
  const dataUrlsBySource = new Map<string, Promise<string>>();
  const loadResource = (source: string) => {
...
function originalResourceFromSvgImage(svg: Element, dimensions: { width: number; height: number }) {
...
function describeContentImage(element: Element): ContentImage | null {
