...
function fitImage(imageWidth, imageHeight, viewportWidth, viewportHeight) {
...
export function getImageViewerBounds(transform) {
  return {
...
function clampOffsets(transform) {
...
export function zoomImageViewerAt(transform, requestedScale, point = {}) {
  const scale = clamp(
...
function resistOffset(value, bound) {
...
export function panImageViewerBy(transform, deltaX, deltaY, { resist = false } = {}) {
  const next = {
    ...transform,
...
export function settleImageViewerTransform(transform) {
  return clampOffsets({
    ...transform,
    scale: clamp(
...
export function resizeImageViewerTransform(transform, {
  viewportHeight,
  viewportWidth,
}) {
  const normalizedViewportWidth = positive(viewportWidth);
