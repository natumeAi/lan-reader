import type { EpubSection, ReaderBook, ReadingSection, TocInput, TocItem } from '../types/epub.js';
type TocMetadata = Pick<TocItem, 'startCfi' | 'startProgress'>;
interface LocationBoundary { cfi: string; locationIndex: number; sectionIndex: number }
...
function hrefFragment(href: string) {
  if (typeof href !== 'string') return '';
...
function readLocationCfis(book: ReaderBook) {
  try {
    const savedLocations = book?.locations?.save?.();
    const locations = typeof savedLocations === 'string'
      ? JSON.parse(savedLocations)
      : savedLocations;
    return Array.isArray(locations) ? locations.filter((value): value is string => typeof value === 'string') : [];
  } catch {
    return [];
  }
}

function locationBoundaries(book: ReaderBook) {
  const boundaries: LocationBoundary[] = [];
  const firstCfiBySection = new Map<number, string>();
...
function fallbackSectionCfi(section: EpubSection, boundaries: LocationBoundary[], firstCfiBySection: Map<number, string>) {
...
function findFragmentElement(document: Document | undefined, fragment: string) {
...
function progressFromCfi(book: ReaderBook, cfi: string | null) {
  if (!cfi) return null;

  try {
    const progress = book?.locations?.percentageFromCfi?.(cfi);
    return typeof progress === 'number' && Number.isFinite(progress) ? Math.min(1, Math.max(0, progress)) : null;
  } catch {
    return null;
  }
}

function applyTocMetadata(items: TocItem[], metadata: Map<string, TocMetadata>): TocItem[] {
...
function fillMissingMetadata(flatItems: TocItem[], metadata: Map<string, TocMetadata>) {
...
export async function addTocProgress(items: TocItem[], book: ReaderBook, { shouldStop = () => false }: { shouldStop?: () => boolean } = {}) {
  const flatItems = flattenTocItems(items);
  const metadata = new Map<string, TocMetadata>();
  const fragmentItemsBySection = new Map<EpubSection, { fragment: string; item: TocItem }[]>();
  ...
  for (const [section, sectionItems] of fragmentItemsBySection) {
    ...
    try {
      ...
      sectionItems.forEach(({ fragment, item }) => {
        ...
        const startCfi = section.cfiFromElement?.(element) || null;
        metadata.set(item.chapterId, {
          startCfi,
          startProgress: progressFromCfi(book, startCfi),
        });
