...
interface Entry {
  ...
  state: 'loading' | 'ready' | 'stale' | 'retiring'; leases: number; working: boolean;
  prepared: PreparedSurface | null; ready: Promise<boolean>; disposal?: Promise<void>;
}
...
export function createPageBuffer(provider: SurfaceProvider) {
  ...
  const dispose = (entry: Entry) => {
    if (entry.disposal || entry.leases || motion) return;
  ...
  const prepare = async (request: SurfaceRequest): Promise<boolean> => {
    ...
    entry.ready = (async () => {
      ...
      try { prepared = await owner.ready; await owner.drain(); } catch { prepared = null; }
      entry.working = false;
  ...
  const acquire = (request: SurfaceRequest): SurfaceLease | null => {
    ...
    return {
      ...
      release() {
        ...
        if (entry.state === 'stale') dispose(entry);
      },
    };
  };
  const invalidate = () => {
    ...
    for (const entry of entries) {
      ...
      entry.state = 'stale';
      if (!entry.leases) { entry.owner.stop(); dispose(entry); }
    }
  };
  ...
  return {
    ...
    endMotion() {
      motion = false;
      for (const entry of entries) if (entry.state === 'stale') dispose(entry);
    },
