...
import type { TocItem as TocEntry } from '../../types/epub.js';
interface TocPanelProps { currentChapterId?: string | null; currentHref?: string | null; onSelect: (href: string) => void; toc: TocEntry[] }
import { useLayoutEffect, useRef } from 'react';
import { centerTocEntry } from '../../utils/tocScrollPosition.js';

function hrefDocument(href: string) {
...
function formatStartProgress(progress: number | null) {
  if (progress === null || !Number.isFinite(progress)) return '…';
  return `${Math.round(Math.min(1, Math.max(0, progress)) * 100)}%`;
}

export function TocPanel({ currentChapterId, currentHref, onSelect, toc }: TocPanelProps) {
...
function TocItem({
  ...
  item,
  onSelect,
}: Omit<TocPanelProps, 'toc'> & { currentEntryRef: RefObject<HTMLButtonElement | null>; depth: number; item: TocEntry }) {
